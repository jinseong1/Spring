...
public class StringMethod {
	...
	public static void main(String[] args) {
		...
		//방법1]Integer클래스의 메소드 사용-인스턴스형 메소드
		Integer inst=Integer.valueOf(lastJumin);//int ->Integer
		String lastString=inst.toString();//inst참조하는 메모리의 데이타(1234567)를 문자열로 변경
		System.out.println(lastString.charAt(0)=='2' ?"여자":"남자");
		//방법2]Integer클래스의 메소드 사용-정적 메소드
		//정적 메소드:Integer.toString(int)
		lastString=Integer.toString(lastJumin);//int ->String
		System.out.println(lastString.charAt(0)=='2' ?"여자":"남자");
		//방법3]오토박싱 사용
		inst = lastJumin;//int ->Integer
		System.out.println(inst.toString().charAt(0)=='1' ?"남자":"여자");
		//3]int codePointAt(int index) :
		//문자열에서 index에 해당하는 아스키(유니코드) 코드값 반환
		System.out.println("str1의 4번째문자의 아스키(유니코드) 값 : "+str1.codePointAt(3));
		System.out.println("str2의 4번째문자의 아스키(유니코드) 값 : "+Integer.toHexString(str2.codePointAt(3)));
		System.out.println(WrapperClass03.isNumber("100") ?"숫자형식이다" :"숫자 형식이 아니다");
		System.out.println(WrapperClass03.isNumber("10A") ?"숫자형식이다" :"숫자 형식이 아니다");
		...
		if("Hi Java".indexOf("Hello") !=-1) {
			System.out.println("Hi Java!".replace("Hello","안녕"));
			System.out.println("'Hello'를 '안녕'으로 변경");
		}
		else System.out.println("'Hello'라는 문자열이 없어요");
		...
		email="kim@naver.com";
		strArr=email.split("\\.");//.은 정규표현식이 아니다라는 것을 알려주기위해 \\ 를 붙인다
		for(int i=0;i < strArr.length;i++)
			System.out.println(String.format("strArr[%d]=%s",i,strArr[i]));
		...
		//  메소드
		 ...
		 String filename="my.file.homeowrk.ppt";
