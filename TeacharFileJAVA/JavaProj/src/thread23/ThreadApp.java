...
class DaemonThread extends Thread{
	...
	public void run() {
		while(true) {
			...
			try {
				sleep(3000);
				System.out.println(String.format("%s]2초마다 저장",getName()));
			} catch (InterruptedException e) {e.printStackTrace();}
		}
	}
}///////////////////////////////
...
public class ThreadApp {
	...
	public static void main(String[] args)  {
		...
		*/
		...
		System.out.println("현재 활성화 상태(Runnable혹은 Running상태) 있는 스레드수:"+Thread.activeCount());
		System.out.println("첫번째 스레드의 우선권:"+yt1.getPriority());
		System.out.println("두번째 스레드의 우선권:"+yt2.getPriority());
		System.out.println("첫번째 스레드의 종속 스레드 여부:"+(yt1.isDaemon()?"종속스레드":"독립스레드"));
		System.out.println("daemon의 종속 드레드 여부:"+(daemon.isDaemon() ? "종속스레드":"독립스레드"));
		System.out.println("main스레드의 스레드 우선권:"+Thread.currentThread().getPriority());
