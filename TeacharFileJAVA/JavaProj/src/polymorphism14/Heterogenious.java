...
class Child extends Parent{
	...
	void parentMethod(int childMember) {
		System.out.println("자식에서 오로딩한 메소드:parentMethod(int childMember)");
	}
...
public class Heterogenious {
	...
	public static void main(String[] args) {
		...
		//[멤버 메소드류]
		child.childMethod();//새롭게 추가
		child.parentMethod();//오버라이딩
		child.parentMethod(10);//오버로딩
		System.out.println("[Parent타입에 자식 타입의 메모리 주소 복사]");
		Parent parent = child;//Heterogenious]
		System.out.println(parent.parentMember);
		parent.parentMethod();//오버라이딩한 메소드
		...
		//방법1]형변환후 바로 접근
		System.out.println(((Child)parent2).childMember);
		((Child)parent2).childMethod();
		((Child)parent2).parentMethod(10);
